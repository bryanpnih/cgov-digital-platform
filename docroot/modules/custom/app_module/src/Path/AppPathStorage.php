...
class AppPathStorage implements AppPathStorageInterface {
  ...
  public function save(
    ...
    if ($pid) {
      /*
       * Do we need to clear any caches?
  ...
  public function load($conditions) {
    ...
    try {
      ...
      // Deserialize the data field from results.
      return array_map(
        function ($instance) {
          $instance['app_module_data'] = unserialize($instance['app_module_data']);
          return $instance;
        },
        $result
      );
  ...
  public function pathExists($app_module_id, $alias, $langcode, $source = NULL) {
    ...
    $query = $this->connection->select(static::TABLE)
      ->condition('app_module_id', $app_module_id)
